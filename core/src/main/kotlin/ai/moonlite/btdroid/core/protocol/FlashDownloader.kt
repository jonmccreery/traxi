...
class FlashDownloader(
    ...
    suspend fun download(
        ...
        while (address < maxBytes && shouldContinue()) {
            ...
            // A short block usually means a dropped chunk, not end of flash.
            // Retry once at full size before accepting what we got.
            if (!block.isComplete) {
                retries++
                client.transcript.note(
                    "block 0x%08X short (%d/%d), retrying".format(address, block.filled, blockSize)
                )
                block = client.readLogBlock(address, blockSize)
            }

            if (block.filled == 0) {
                client.transcript.note("no data at 0x%08X; treating as end of flash".format(address))
                break
            }
        ...
        return Result(out.toByteArray(), sectors, retries, stoppedOnUnwritten)
    }
